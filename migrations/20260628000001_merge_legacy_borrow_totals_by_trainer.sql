...
--
-- 20260628000000 has already been deployed and must remain checksum-stable.
-- It copied v1 rows by the old inheritance_id/support_card_id identity, but
-- that identity is not stable enough to be a historical borrow key. Keep v1
-- untouched and aggregate all legacy history by trainer_id into one derived v2
-- row. Readers merge this transition row with the current bk1 row.
...
ON CONFLICT (trainer_id, borrow_key) DO UPDATE SET
    ...
    last_viewed_at = COALESCE(
        GREATEST(borrow_interaction_totals_v2.last_viewed_at, EXCLUDED.last_viewed_at),
        borrow_interaction_totals_v2.last_viewed_at,
        EXCLUDED.last_viewed_at
    ),
    last_copied_at = COALESCE(
        GREATEST(borrow_interaction_totals_v2.last_copied_at, EXCLUDED.last_copied_at),
        borrow_interaction_totals_v2.last_copied_at,
        EXCLUDED.last_copied_at
    ),
    last_recheck_at = COALESCE(
        GREATEST(borrow_interaction_totals_v2.last_recheck_at, EXCLUDED.last_recheck_at),
        borrow_interaction_totals_v2.last_recheck_at,
        EXCLUDED.last_recheck_at
    ),
    last_recheck_task_created_at = COALESCE(
        GREATEST(
