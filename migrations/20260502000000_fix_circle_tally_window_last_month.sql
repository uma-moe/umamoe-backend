-- Keep circle rankings on last-month values from 00:00 on the 2nd JST until
-- 00:00 on the 3rd JST. The 1st JST still belongs to the previous game month,
-- and new-month ranking data is not reliable for display until the first full
-- tally window has completed.
...
SELECT
    ...
    CASE
        WHEN tb.now_jst >= tb.game_month_start_jst
         AND tb.now_jst < tb.tally_window_end_jst
