-- Fix circle_live_ranks to use live_points only when last_updated is fresh (<1h),
-- falling back to monthly_point for stale circles.
...
SELECT
    ...
    RANK() OVER (
        ORDER BY CASE
            ...
            THEN GREATEST(live_points, monthly_point)
            ELSE monthly_point
        END DESC NULLS LAST
    ) AS live_rank,
    RANK() OVER (
